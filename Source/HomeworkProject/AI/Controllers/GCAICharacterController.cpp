...
void AGCAICharacterController::SetPawn(APawn* InPawn)
{
	...
	if (IsValid(InPawn))
	{
		...
		RunBehaviorTree(CachedAICharacter->GetBahaviorTree());//запускаем дерево
	}
	else
	{
...
void AGCAICharacterController::TryMoveToNextTarget()
{
	...
	if (IsValid(ClosestActor))
	{
		if (IsValid(Blackboard))
		{
			Blackboard->SetValueAsObject(BB_CurrentTarget, ClosestActor);//задаём в переменую blackboard-а значение(1 - имя, 2 - значение)
			SetFocus(ClosestActor, EAIFocusPriority::Gameplay);//позволяет установить ориентацию, то куда будет смотреть персонаж
